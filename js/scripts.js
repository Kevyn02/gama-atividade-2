...
function criandoCards() {
    ...
    for (var i = 0; i < json.length; i++) {
        nome[i].innerHTML = json[i]['name'];
        tipo[i].innerHTML = 'Tipo: ' + json[i]['property_type'];
        preco[i].innerHTML = 'R$ ' + json[i]['price'] + '/noite';
        image[i].setAttribute('src', json[i]['photo']);
    }
...
function criandoOptions() {
    ...
    for (var i = 0; i < json.length; i++) {
        if (tipo.length == 0) {
            tipo.push(json[i]['property_type']);
        } else if (tipo.indexOf(json[i]['property_type']) == -1) {
            tipo.push(json[i]['property_type']);
        }
    }
    console.log(tipo);
